    ...
    public function add_faq()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['edit_faq']) && !empty($_POST['edit_faq'])) {
                    if (is_exist(['question' => $_POST['question'], 'status' => '1'], 'faqs', $_POST['edit_faq'])) {
                        sendWebJsonResponse(true, 'Question Already Exist !');
                    }
                } else {
                ...
                $fields = ['edit_faq', 'question', 'answer'];

                foreach ($fields as $field) {
                    $faq_data[$field] = $this->input->post($field, true) ?? "";
                }

                $this->faq_model->add_faq($faq_data);
