    ...
    public function update_privacy_policy_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->Setting_model->update_admin_privacy_policy($privacy_policy_input_description);
                $this->Setting_model->update_admin_terms_n_condtions($terms_n_conditions_input_description);
                sendWebJsonResponse(false, 'Admin Privacy Policy & Terms and Conditions Updated Successfully');
            }
        } else {
            redirect('admin/login', 'refresh');
        }
