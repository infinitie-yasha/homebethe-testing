...
class Pickup_location extends CI_Controller
{
    ...
    public function add_pickup_location()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $message = (isset($_POST['edit_pickup_location'])) ? 'Pickup location Updated' : 'Pickup location Added';
    ...
    public function delete_pickup_location()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $pickup_location = $this->input->get('id', true);
            $product_data = fetch_details('products', ['pickup_location' => $pickup_location]);

            if (isset($product_data) && !empty($product_data)) {
                sendWebJsonResponse(true, 'This pickup location cannot be deleted as it is currently assigned to some products.');
            }

            if (delete_details(['id' => $_GET['id']], 'pickup_locations') == TRUE) {
                sendWebJsonResponse(false, 'Pickup Location Deleted Successfully');
            } else {
