...
class Sellers extends CI_Controller
{
    ...
    public function manage_seller()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $all_payment_gateways = get_settings('payment_method', true);
            $available_payment_gateways = get_available_payment_gateways($all_payment_gateways);
            $this->data['available_payment_gateways'] = $available_payment_gateways;
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (isset($_GET['seller_status']) && !empty($_GET['seller_status'])) {
    ...
    public function remove_sellers()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!isset($_GET['id']) && empty($_GET['id'])) {
                sendWebJsonResponse(true, 'Seller id is required');
            }
            $all_status = [0, 1, 2, 7];
            $status = $this->input->get('status', true);
            $id = $this->input->get('id', true);
            if (!in_array($status, $all_status)) {
                sendWebJsonResponse(true, 'Invalid status');
            }
            if ($status == 2) {
                sendWebJsonResponse(true, 'Please approve seller first for remove seller.');
            }

            // if seller is removed(7) then pass 1 for restore seller , if active(1) then pass 7 for remove seller
            $status = ($status == 7) ? 1 : (($status == 1) ? 7 : 1);

            if (update_details(['status' => $status], ['user_id' => $id], 'seller_data') == TRUE) {
                sendWebJsonResponse(false, 'Seller removed succesfully');
            } else {
    ...
    public function add_seller()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('store_url', 'Store URL', 'trim|xss_clean|valid_url');
            ...
            $this->form_validation->set_rules('store_url', 'Store URL', 'trim|xss_clean|regex_match[/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w\.-]*)*\/?$/]');
            //seo validation
            ...
            $this->form_validation->set_rules('bank_ifsc', 'Bank IFSC', 'trim|required|xss_clean');
            $this->form_validation->set_rules('bank_name', 'Bank Name', 'trim|required|xss_clean');
            $this->form_validation->set_rules('bank_acc', 'Bank Account', 'trim|required|xss_clean');
            ...
            if (!isset($_POST['edit_seller'])) {
                if (isset($_POST['global_commission']) && empty($_POST['global_commission'])) {
                    $this->form_validation->set_rules('commission_data', 'Category Commission data or Global Commission', 'trim|required|xss_clean|numeric|greater_than_equal_to[0]');
                }
            }
            ...
            $base_upload_path = rtrim(FCPATH, '/') . '/' . trim(MEDIA_PATH, '/') . '/' . $year . '/';
            $license_upload_path = $base_upload_path . 'pan_image/';
            ...
            // Create pan_image folder
            if (!is_dir($license_upload_path)) {
                mkdir($license_upload_path, 0777, true);
            }
            ...
            if (!empty($_FILES['pan_image']['name'])) {
                ...
                $config = [
                    'upload_path' => $license_upload_path, // filesystem path
                ...
                $pan_image = MEDIA_PATH . $year . '/pan_image/' . $upload_data['file_name'];
            }
            ...
            if ($store_logo_error != NULL) {
                sendWebJsonResponse(true, $store_logo_error);
            }
            ...
            if ($authorized_signature_error != NULL) {
                sendWebJsonResponse(true, $authorized_signature_error);
            }
            ...
            if ($id_card_error != NULL) {
                sendWebJsonResponse(true, $id_card_error);
            }
            ...
            if ($proof_error != NULL) {
                ...
                sendWebJsonResponse(true, $proof_error);
            }
            ...
            $permmissions = array();
            // changed because of customization,every seller product approval is mandatory
            ...
            if (isset($_POST['edit_seller']) && !empty($_POST['edit_seller'])) {
                ...
                $seller_data = array(
                    ...
                    "bank_code" => $this->input->post('bank_ifsc', true),
                    "bank_name" => $this->input->post('bank_name', true),
                    "account_number" => $this->input->post('bank_acc', true),
                ...
                if ($this->Seller_model->add_seller($seller_data, $seller_profile, $com_data)) {
                    ...
                    sendWebJsonResponse(false, $message, ['redirect_url' => base_url('admin/sellers/')]);
                } else {
                ...
                if (!$this->form_validation->is_unique($_POST['mobile'], 'users.mobile') || !$this->form_validation->is_unique($_POST['email'], 'users.email')) {
                    sendWebJsonResponse(true, 'Email or mobile already exists !');
                }
                ...
                if (update_details(['active' => 1], [$identity_column => $identity], 'users')) {
                    ...
                    if (!empty($insert_id)) {
                        ...
                        sendWebJsonResponse(false, 'Seller Added Successfully', ['redirect_url' => base_url('admin/sellers')]);
                    } else {
                        sendWebJsonResponse(true, 'Seller data was not added');
                    }
                } else {
                    $message = (isset($_POST['edit_seller']) && !empty($_POST['edit_seller'])) ? 'Seller not Updated' : 'Seller not Added.';
                    sendWebJsonResponse(true, $message);
                }
            }
        } else {
    ...
    public function get_seller_commission_data()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (empty($result)) {
                sendWebJsonResponse(true, 'No category & commission data found for seller.');
            } else {
                sendWebJsonResponse(false, '', [], $result);
            }
        } else {
    ...
    public function create_slug()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!empty($sellers)) {
                ...
                sendWebJsonResponse(false, 'Slug Created Successfully');
            } else {
                sendWebJsonResponse(true, 'Already Created No need to create again.');
            }
        } else {
            redirect('admin/login', 'refresh');
        }
