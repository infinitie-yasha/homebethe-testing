    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                $this->response['fetched_data'] = $this->db->select(' u.* ')
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            foreach ($order_items as $order_item) {
                ...
                if ($order_item_status != 'delivered' || $order_item_status != 'returned' || $order_item_status != 'cancelled') {
                    sendWebJsonResponse(true, 'You cannot delete delivery boy , orders is not delivered, please once assign to other delivery boy or deliver all the orders ');
                }
            }
            if (delete_details(['user_id' => $_GET['id']], 'users_groups')) {
                ...
                sendWebJsonResponse(false, 'User removed from delivery boy successfully');
            } else {
    ...
    public function add_delivery_boy()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_POST['bonus_type']) && $_POST['bonus_type'] == 'fixed_amount_per_order_item') {
                $this->form_validation->set_rules('bonus_amount', 'Bonus Amount', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]');
                $bonus = $this->input->post('bonus_amount');
            } elseif (isset($_POST['bonus_type']) && $_POST['bonus_type'] == 'percentage_per_order_item') {
                $this->form_validation->set_rules('bonus_percentage', 'Bonus Percentage', 'trim|required|numeric|xss_clean|greater_than_equal_to[0]|less_than_equal_to[100]');
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($images_info_error != NULL) {
                    sendWebJsonResponse(true, $images_info_error);
                }
                if (isset($_POST['edit_delivery_boy']) && !empty($_POST['edit_delivery_boy'])) {
                ...
                $message = (isset($_POST['edit_delivery_boy']) && !empty($_POST['edit_delivery_boy'])) ? 'Delivery Boy Update Successfully' : 'Delivery Boy Added Successfully';
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!is_exist(['id' => $delivery_boy_id], 'users')) {
                    ...
                    $message = 'Delivery Boy is not exist in your database';
                    sendWebJsonResponse(true, $message);
                }
                ...
                if ($res[0]['cash_received'] < $amount) {
                    $message = 'Amount must be not be greater than cash';
                    sendWebJsonResponse(true, $message);
                }
                ...
                if ($res[0]['cash_received'] > 0 && $res[0]['cash_received'] != null) {
                    ...
                    $error = false;
                    $message = 'Amount Successfully Collected';
                } else {
                    $error = true;
                    $message = 'Cash should be greater than 0';
                }
