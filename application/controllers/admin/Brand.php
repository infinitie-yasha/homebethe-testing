...
class Brand extends CI_Controller
{
    ...
    public function add_brand()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_POST['edit_brand']) && !empty($_POST['edit_brand'])) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!isset($_POST['edit_brand']) || empty($_POST['edit_brand'])) {
                    if (is_exist(['name' => $_POST['brand_input_name']], 'brands')) {
                        ...
                        sendWebJsonResponse(true, 'Brand Already exist you should use a different name');
                    }
                }
                ...
                $message = (isset($_POST['edit_brand']) && !empty($_POST['edit_brand'])) ? 'Brand Updated Successfully' : 'Brand Added Successfully';
    ...
    function delete_brand()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if ($this->Brand_model->delete_brand($_GET['id']) == TRUE) {
                ...
                sendWebJsonResponse(false, 'Brand Deleted Successfully');
            }
        } else {
            redirect('admin/login', 'refresh');
        }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($type == 'upload') {
                        ...
                        if ($temp != 0) {
                            ...
                            if (!empty($row[0])) {
                                if (is_exist(['name' => $row[0]], 'brands')) {
                                    sendWebJsonResponse(true, "brand Already Exist! Provide another brand name at row." . $temp);
                                }
                            }
                            if (empty($row[1])) {
                    ...
                    sendWebJsonResponse(false, 'brands uploaded successfully!');
                } else { // bulk_update
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row vales
                    {
                        if ($temp != 0) {
                            if (empty($row[0])) {
                                sendWebJsonResponse(true, 'brand id is empty at row ' . $temp);
                            }
                            if (!empty($row[0])) {
                                if (!is_exist(['id' => $row[0]], 'brands')) {
                                    sendWebJsonResponse(true, "brand is not exist Provide another brand id at row." . $temp);
                                }
                            }
                            if (empty($row[1])) {
                                sendWebJsonResponse(true, 'Name is empty at row ' . $temp);
                    ...
                    fclose($handle);
                    sendWebJsonResponse(false, 'brands updated successfully!');
                }
            }
        } else {
            redirect('admin/login', 'refresh');
