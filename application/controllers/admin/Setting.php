...
class Setting extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($settings['single_seller_system_seller_id']) && !empty($settings['single_seller_system_seller_id'])) {
                $where = ['u.id' => $settings['single_seller_system_seller_id']];
            }
            ...
            if (isset($where) && !empty($where)) {
                $this->data['seller'] = $this->data['seller']->where($where);
            }
            $this->data['seller'] = $this->data['seller']->get('users u')->result_array();
    ...
    public function update_system_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_POST['single_seller_system']) && !empty($_POST['single_seller_system']) && $_POST['single_seller_system'] == 'on') {

                $this->form_validation->set_rules('single_seller_system_seller_id', 'Single Seller System Seller', 'trim|required|numeric|xss_clean');
            }

            if (isset($_POST['is_customer_app_under_maintenance']) && $_POST['is_customer_app_under_maintenance']) {
                $this->form_validation->set_rules('message_for_customer_app', 'Message for Customer App', 'trim|required|xss_clean');
            }
            if (isset($_POST['is_seller_app_under_maintenance']) && $_POST['is_seller_app_under_maintenance']) {
                $this->form_validation->set_rules('message_for_seller_app', 'Message for Seller App', 'trim|required|xss_clean');
            }
            if (isset($_POST['is_delivery_boy_app_under_maintenance']) && $_POST['is_delivery_boy_app_under_maintenance']) {
                $this->form_validation->set_rules('message_for_delivery_boy_app', 'Message for Delivery Boy App', 'trim|required|xss_clean');
            }
            if (isset($_POST['is_web_under_maintenance']) && $_POST['is_web_under_maintenance']) {
                $this->form_validation->set_rules('message_for_web', 'Message for Web', 'trim|required|xss_clean');
            }
    ...
    public function update_web_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                sendWebJsonResponse(false, 'Web Settings Updated Successfully!');
            }
        } else {
            redirect('admin/login', 'refresh');
        }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_POST['update_seller_flow']) && !empty($_POST['update_seller_flow'])) {
                ...
                if ($shipping_data['pincode_wise_deliverability'] == '1') {
                    ...
                    if ($has_error) {
                        sendWebJsonResponse(true, 'Please add serviceable zipcodes for all sellers.');
                    } else {
                        sendWebJsonResponse(false, 'serviceable zipcodes already added all sellers.');
                    }
                    ...
                    // Check if any error was found
                    if ($has_error) {
                        sendWebJsonResponse(true, 'Please add serviceable cities for all sellers.');
                    } else {
                        sendWebJsonResponse(false, 'Serviceable Cities already added for all sellers.');
                    }
                }
            }
            die;
