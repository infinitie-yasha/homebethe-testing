...
class Tickets extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $ticket_types = fetch_details('ticket_types', [], 'id,title');
    ...
    public function add_ticket_type()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules(
                'type_for',
                'Type',
                'trim|required|in_list[seller,customer]|xss_clean',
            ...
            if (!$this->form_validation->run()) {
                ...
                $ticket['type_for'] = $this->input->post('type_for', true);

                $this->ticket_model->add_ticket_type($ticket);

                $message = (isset($_POST['edit_ticket_type']) && !empty($_POST['edit_ticket_type'])) ? 'Ticket Types Updated Successfully' : 'Ticket Types Added Successfully';
    ...
    public function view_ticket_list()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            return $this->ticket_model->get_ticket_list($ticket_type_filter);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!empty($insert_id)) {
                    ...
                    sendWebJsonResponse(false, 'Ticket message sent successfully', [], $this->response);
                } else {
                    sendWebJsonResponse(true, 'Ticket message could not be sent!', [], $this->response);
    ...
    public function edit_ticket_status()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (empty($res)) {
                    sendWebJsonResponse(true, 'User id is changed you can not udpate the ticket.');
                }
                if ($status == PENDING && $res[0]['status'] == OPENED) {
                    sendWebJsonResponse(true, 'Current status is opened.');
                }
                if ($status == OPENED && ($res[0]['status'] == RESOLVED || $res[0]['status'] == CLOSED)) {
                    sendWebJsonResponse(true, 'Can\'t be OPEN but you can REOPEN the ticket.');
                }
                if ($status == RESOLVED && $res[0]['status'] == CLOSED) {
                    sendWebJsonResponse(true, 'Current status is closed.');
                }
                if ($status == REOPEN && ($res[0]['status'] == PENDING || $res[0]['status'] == OPENED)) {
                    sendWebJsonResponse(true, 'Current status is pending or opened.');
                }
                ...
                if (!$ticket_data) {
                    ...
                    sendWebJsonResponse(false, 'Ticket updated Successfully', [], $result_data);
                } else {
                    $result_data = (!empty($this->response['data'])) ? $this->response['data'] : [];
                    sendWebJsonResponse(true, 'Ticket Not Added', [], $result_data);
                }
            }
        } else {
            redirect('admin/login', 'refresh');
