...
class Shipping_settings extends CI_Controller
{
    ...
    public function update_shipping_settings()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('default_delivery_charge', 'Default Delivery Charge', 'trim|required|numeric|xss_clean');
            $this->form_validation->set_rules('minimum_free_delivery_order_amount', 'Minimum Free Delivery Order Amount', 'trim|required|numeric|xss_clean');

            if ((!isset($_POST['shiprocket_shipping_method']) && !isset($_POST['local_shipping_method']))) {
                sendWebJsonResponse(true, 'Please select atleast one shipping method');
            }
            ...
            if (!isset($selected_method) && empty($selected_method)) {
                sendWebJsonResponse(true, 'Please select atleast one deliverability system');
            }
            if (!$this->form_validation->run()) {
                ...
                sendWebJsonResponse(false, 'Shipping Setting Updated successfully.');
            }
        } else {
            redirect('admin/login', 'refresh');
        }
