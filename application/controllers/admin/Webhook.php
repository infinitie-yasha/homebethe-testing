...
class Webhook extends CI_Controller
{
    ...
    public function razorpay()
    {
        ...
        if ($request['event'] == 'payment.captured' || $request['event'] == 'order.paid') {
            ...
            if ($request['event'] == 'order.paid') {
                ...
                $order_id = $order_id ?? $request['payload']['payment']['entity']['notes']['order_id'];
                    ...
                    if (isset($order['order_data'][0]['user_id'])) {
                        ...
                        if (!empty($user_fcm)) {
                            ...
                            if (!empty($fcms)) {
                                ...
                                $user_fcm_id[0] = $user_fcm_ids ?? $user_fcm_ids[0]['fcm_id'];
                        ...
                        foreach ($order['order_data'] as $item) {
                            foreach ($item['order_items'] as $order_item) {
                                $product_variant_ids[] = $order_item['product_variant_id'];
                                $quantity[] = $order_item['quantity'];
                            }
                        }

                        update_stock($product_variant_ids, $quantity, 'minus');
                    }
                }
            } else {
    ...
    public function myfatoorah()
    {
        ...
        if (empty($data['Event'])) {
            ...
            log_message('error', $data['Data']['Transaction']['PaymentId']);
            log_message('error', "amount : " . $data['Data']['Amount']['ValueInBaseCurrency']);
            $txn_id = (isset($data['Data']['Transaction']['PaymentId'])) ? $data['Data']['Transaction']['PaymentId'] : "";

            if (!empty(($data['Data']['Transaction']['PaymentId']))) {
                if (!empty($txn_id)) {
                    $transaction = fetch_details('transactions', ['txn_id' => $txn_id], '*');
                }
                $amount = $data['Data']['Amount']['ValueInBaseCurrency'];
                // $amount = ($amount / 100);
            } else {
                $amount = 0;
            ...
            if (!empty($transaction)) {
                ...
                $order_id = 0;
                $order_id = (isset($data['Data']['Invoice']['UserDefinedField'])) ? $data['Data']['Invoice']['UserDefinedField'] : $data['Data']['Invoice']['UserDefinedField'];
            }
            ...
            //     return;
            // }
            if ($data['Event']['Name'] == 'PAYMENT_STATUS_CHANGED') {
                if ($data['Data']['Transaction']['Status'] == 'Failed') {
                    ...
                    $response['error'] = true;
                    $response['transaction_status'] = $data['Data']['Transaction']['Status'];
                    $response['message'] = "Transaction is failed. ";
                    log_message('error', 'Razorpay Webhook | Transaction is failed --> ' . var_export($data['Data']['Transaction']['Status'], true));
                    echo json_encode($response);
                    return false;
                } else if ($data['Data']['Transaction']['Status'] == 'SUCCESS') {


                    if ($data['Event'] == 'TransactionsStatusChanged' || $data['Event'] == 'PAYMENT_STATUS_CHANGED') {
                    ...
                    if (!empty($order_id)) {
                        ...
                        if (strpos($order_id, "wallet-refill-user") !== false) {
                            ...
                            if ($this->customer_model->update_balance($amount, $user_id, 'add')) {
                                ...
                                $response['transaction_status'] = $data['Data']['Transaction']['Status'];
                                $response['message'] = "Wallet could not be recharged!";
                                log_message('error', 'My Fatoorah Webhook | wallet recharge failure --> ' . var_export($data['Data']['Transaction']['Status'], true));
                            }
                            echo json_encode($response);
                            return false;
                        } else {
                            ...
                            if (isset($order['order_data'][0]['user_id'])) {
                                ...
                                if (!empty($transaction)) {
                                    ...
                                    /* add transaction of the payment */
                                    $amount = ($data['Data']['ValueInBaseCurrency']);
                                    $data = [
                                        'transaction_type' => 'transaction',
                                        'user_id' => $user_id,
                                        'order_id' => $order_id,
    ...
    //         log_message('error', 'phonepe server webhook--> ' . var_export($request, true));
    //         log_message('error', 'phonepe meta info--> ' . var_export($request['metaInfo'], true));
    //         log_message('error', 'phonepe meta info--> ' . var_export($request['metaInfo']['merchantTransactionId'], true));
    //         log_message('error', 'phonepe meta info--> ' . $request['metaInfo']['merchantTransactionId']);

    //         $txn_id = (isset($request['metaInfo']['merchantTransactionId'])) ? $request['metaInfo']['merchantTransactionId'] : "";
    ...
    //             }
    //             $amount = $request['metaInfo']['amount'] / 100;
    //         } else {
    //             $amount = 0;
    //         }
    //         if (!empty($transaction)) {
    ...
    //         $status = (isset($request['state'])) ? $request['state'] : "";

    //         $this->load->model('transaction_model');
    //         log_message('error', 'txn id status : '. $txn_id);

    //         $check_status = $this->phonepe->check_status_v2($txn_id);
    //         log_message('error', 'check status : '. var_export($check_status,1));
    //         $txn_id = $transaction[0]['txn_id'];
    //         if ($check_status['state'] == 'COMPLETED') {
    //             if ($status == 'COMPLETED') {
    //                 $data['status'] = "success";
    ...
    public function phonepe_webhook()
    {
        ...
        // Adjust as per the actual webhook structure you receive
        $metaInfo = $request['metaInfo'] ?? [];
        $txn_id = $metaInfo['merchantTransactionId'] ?? '';
        $amount = isset($metaInfo['amount']) ? $metaInfo['amount'] / 100 : null;
        $status = $request['state'] ?? null;

        if ($txn_id) {
            $response = process_phonepe_result($txn_id, $amount, $status);
        } else {
            log_message('error', 'Transaction ID missing in webhook');
        }
    }
}
