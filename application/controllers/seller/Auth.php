...
class Auth extends CI_Controller
{
    ...
    public function get_categories()
    {
        ...
        $response = $this->data['category_result'] = $this->category_model->get_categories(NULL, '', '', 'row_order', 'ASC', 'true', '', $ignore_status, $seller_id, $from_select);
    ...
    public function create_seller()
    {
        ...
        // pan_number, bank_ifsc , bank_name , bank_acc
        ...
        // Only enforce password strength when creating a new seller AND a non-empty password is provided
        $passwordToCheck = $this->input->post('password', TRUE);
        $userIdPost = $this->input->post('user_id', TRUE);


        if (empty($userIdPost) && $passwordToCheck !== null && trim($passwordToCheck) !== '') {
            if (!preg_match($regex_password, $passwordToCheck)) {
        ...
        if (!$this->form_validation->run()) {
            ...
            // $categoryArray = explode(',', $selected_categories ?? '');  // categories must be in array
            ...
            if (isset($logo_files['store_logo']) && !empty($logo_files['store_logo']['name']) && isset($logo_files['store_logo']['name'])) {
                ...
                // Deleting uploaded image file if any overall error occurred
                if ($store_logo_error != NULL || !$this->form_validation->run()) {
                    if (!empty($store_logo_doc) && is_string($store_logo_doc)) {
                        $filePath = FCPATH . $store_logo_doc;
            ...
            if (isset($authorized_signature_files['authorized_signature']) && !empty($authorized_signature_files['authorized_signature']['name']) && isset($authorized_signature_files['authorized_signature']['name'])) {
                ...
                // Deleting uploaded authorized signature file if any overall error occurred
                if ($authorized_signature_error != NULL || !$this->form_validation->run()) {
                    if (!empty($authorized_signature_doc) && is_string($authorized_signature_doc)) {
                        $filePath = FCPATH . $authorized_signature_doc;
                        if (file_exists($filePath)) {
                            @unlink($filePath);
                        }
                    }
            ...
            if (isset($id_card_files['national_identity_card']) && !empty($id_card_files['national_identity_card']['name']) && isset($id_card_files['national_identity_card']['name'])) {
                ...
                // Deleting uploaded id card file if any overall error occurred
                if ($id_card_error != NULL || !$this->form_validation->run()) {
                    if (!empty($id_card_doc) && is_string($id_card_doc)) {
                        $filePath = FCPATH . $id_card_doc;
                        if (file_exists($filePath)) {
                            @unlink($filePath);
                        }
                    }
            ...
            if (isset($proof_files['address_proof']) && !empty($proof_files['address_proof']['name']) && isset($proof_files['address_proof']['name'])) {
                ...
                // Deleting uploaded address proof file if any overall error occurred
                if ($proof_error != NULL || !$this->form_validation->run()) {
                    if (!empty($proof_doc) && is_string($proof_doc)) {
                        $filePath = FCPATH . $proof_doc;
                        if (file_exists($filePath)) {
                            @unlink($filePath);
                        }
                    }
            ...
            if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
                ...
                if (update_details(['active' => 1], [$identity_column => $identity], 'users')) {
                    ...
                    // Use positional arguments for compatibility with older PHP versions
    ...
    public function check_exists()
    {
        ...
        if (empty($user_exits)) {
            return sendWebJsonResponse(false, "Users does not exists");
        } else {
