...
class Media extends CI_Controller
{
    ...
    public function upload()
    {
        ...
        if (empty($_FILES) || $other_image_info_error != NULL) {
            sendWebJsonResponse(true, (empty($_FILES)) ? "Files not Uploaded Successfully..!" :  $other_image_info_error);
            return;
        }
        ...
        sendWebJsonResponse(false, 'Files Uploaded Successfully..!');
    }
    ...
    public function media_delete()
    {
        ...
        if ($this->input->post('ids')) {
            ...
            if ($deleted) {
               ...
               sendWebJsonResponse(true, 'Media items could not be deleted.');
            }
   ...
   function fetch()
{
    if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
        ...
        // Pass seller_id to model
        return $this->media_model->fetch_media($seller_id);
    } else {
        redirect('seller/login', 'refresh');
    }
}
