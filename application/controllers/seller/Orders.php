    ...
    public function edit_orders()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id']) && !empty($res) && is_numeric($_GET['edit_id'])) {
                ...
                // Compute allow-to-ship flag for UI gating
                $is_allow_to_ship_order = false;
                foreach ($items as $it) {
                    $status = isset($it['active_status']) ? strtolower($it['active_status']) : '';
                    $type = isset($it['product_type']) ? strtolower($it['product_type']) : '';
                    if ($type == 'digital_product') {
                        continue;
                    }
                    // Allow creating parcel when item has moved past awaiting/draft and is not cancelled/delivered
                    if ($status !== 'awaiting' && $status !== 'draft' && $status !== 'cancelled' && $status !== 'delivered') {
                ...
                $this->data['is_allow_to_ship_order'] = $is_allow_to_ship_order;
                ...
                $this->data['pickup_locations'] = fetch_details('pickup_locations', ['status' => 1, 'seller_id' => $seller_id], 'id,name,pickup_location');
                // echo "<pre>";
                // print_r($this->data['pickup_locations']);
                $this->load->view('seller/template', $this->data);
            } else {
