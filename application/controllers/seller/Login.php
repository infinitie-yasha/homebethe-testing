...
class Login extends CI_Controller
{
    ...
    public function update_user()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            $this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|callback_mobile_readonly');
            ...
            if (isset($shipping_method['pincode_wise_deliverability']) && !empty($shipping_method['pincode_wise_deliverability']) && ($shipping_method['pincode_wise_deliverability'] == 1) && isset($shipping_method['local_shipping_method']) && $shipping_method['local_shipping_method'] == 1) {
                if (isset($_POST['deliverable_zipcode_type']) && !empty($_POST['deliverable_zipcode_type']) && ($_POST['deliverable_zipcode_type'] == 3)) {
            ...
            if (isset($shipping_method['city_wise_deliverability']) && !empty($shipping_method['city_wise_deliverability']) && ($shipping_method['city_wise_deliverability'] == 1) && isset($shipping_method['local_shipping_method']) && $shipping_method['local_shipping_method'] == 1) {
                if (isset($_POST['deliverable_city_type']) && !empty($_POST['deliverable_city_type']) && ($_POST['deliverable_city_type'] == 3)) {
            ...
            if (!$this->form_validation->run()) {
                ...
                //process address_proof
                $temp_array_seo_og_image = $seo_og_image_doc = array();
                $seo_og_image_files = $_FILES;
                $seo_og_image_error = "";
                $config = [
                    'upload_path' => FCPATH . SELLER_DOCUMENTS_PATH,
                    'allowed_types' => 'jpg|png|jpeg|gif',
                ...
                if (isset($seo_og_image_files['seo_og_image']) && !empty($seo_og_image_files['seo_og_image']['name']) && isset($seo_og_image_files['seo_og_image']['name'])) {
                    $other_img = $this->upload;
                    $other_img->initialize($config);

                    if (isset($_POST['edit_seller']) && !empty($_POST['edit_seller']) && isset($_POST['old_seo_og_image']) && !empty($_POST['old_seo_og_image'])) {
                        $old_seo_og_image = explode('/', $this->input->post('old_seo_og_image', true));
                        delete_images(SELLER_DOCUMENTS_PATH, $old_seo_og_image[2]);
                    }

                    if (!empty($seo_og_image_files['seo_og_image']['name'])) {

                        $_FILES['temp_image']['name'] = $seo_og_image_files['seo_og_image']['name'];
                        $_FILES['temp_image']['type'] = $seo_og_image_files['seo_og_image']['type'];
                        $_FILES['temp_image']['tmp_name'] = $seo_og_image_files['seo_og_image']['tmp_name'];
                        $_FILES['temp_image']['error'] = $seo_og_image_files['seo_og_image']['error'];
                        $_FILES['temp_image']['size'] = $seo_og_image_files['seo_og_image']['size'];
                        if (!$other_img->do_upload('temp_image')) {
                            $seo_og_image_error = 'Images :' . $seo_og_image_error . ' ' . $other_img->display_errors();
                        } else {
                            $temp_array_seo_og_image = $other_img->data();
                            resize_review_images($temp_array_seo_og_image, FCPATH . SELLER_DOCUMENTS_PATH);
                            $seo_og_image_doc = SELLER_DOCUMENTS_PATH . $temp_array_seo_og_image['file_name'];
                        }
                    } else {
                        $_FILES['temp_image']['name'] = $seo_og_image_files['seo_og_image']['name'];
                        $_FILES['temp_image']['type'] = $seo_og_image_files['seo_og_image']['type'];
                        ...
                        $_FILES['temp_image']['size'] = $seo_og_image_files['seo_og_image']['size'];
                        if (!$other_img->do_upload('temp_image')) {
                            $seo_og_image_error = $other_img->display_errors();
                        }
                    }
                    //Deleting Uploaded Images if any overall error occured
                    if ($seo_og_image_error != NULL || !$this->form_validation->run()) {
                        if (isset($seo_og_image_doc) && !empty($seo_og_image_doc || !$this->form_validation->run())) {
                            foreach ($seo_og_image_doc as $key => $val) {
                                unlink(FCPATH . SELLER_DOCUMENTS_PATH . $seo_og_image_doc[$key]);
                            }
                        }
                    }
                }

                if ($seo_og_image_error != NULL) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = $seo_og_image_error;
                    print_r(json_encode($this->response));
                    return;
                }
                ...
                if ($profile_error != NULL && !empty($profile_error)) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                if (isset($_POST['edit_seller'])) {
                    ...
                    $seller_data = array(
                        ...
                        'seo_og_image' =>  (!empty($seo_og_image_doc)) ? $seo_og_image_doc : $this->input->post('old_seo_og_image', true),
                    ...
                    $update_result = $this->Seller_model->add_seller($seller_data, $seller_profile);
                    ...
                    if ($update_result) {
                        ...
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                        $this->response['message'] = "Profile update failed. Please try again.";
                        print_r(json_encode($this->response));
                    }
                }
            }
