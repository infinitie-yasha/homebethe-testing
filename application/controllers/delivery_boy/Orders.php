...
class Orders extends CI_Controller
{
    ...
    public function view_orders()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_delivery_boy()) {
            ...
            return $this->Order_model->get_return_order_items_list($deliveryBoyId, $_GET['offset'],$_GET['limit']);
        } else {
            redirect('delivery_boy/login', 'refresh');
        }
    }
    ...
    public function update_order_status()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_delivery_boy()) {
            ...
            if ($res['error']) {
                sendWebJsonResponse(true, $res['message']); 
                return false;
            }
            ...
            if ($_GET['status'] == 'delivered') {
                if ($otp_system == 1) {
                    ...
                    if (!validate_otp(otp: $_GET['otp'], consignment_id: $_GET['id'])) {
                        sendWebJsonResponse(true, 'Invalid OTP supplied!');
                        return false;
                    }
                }
            }
            ...
            sendWebJsonResponse(false, 'Order Status Updated Successfully');
            return false;
        } else {
          
            sendWebJsonResponse(true, $this->response['message']);
            return false;
        }
    }
    ...
    function update_return_order_item_status()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_delivery_boy()) {
            ...
            // Send response
            if ($result) {
                sendWebJsonResponse(false, 'Status Updated Successfully', $result);
            } else {
                sendWebJsonResponse(true, 'Failed to update status');
            }
        }
    }
}
