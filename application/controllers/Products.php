...
class Products extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // If invalid page number, remove it and rebuild URL
        if (!is_numeric($page_no) || $page_no < 1) {
            unset($queryParams['page']); // remove the "page" param
            $newQuery = http_build_query($queryParams);
            $redirectUrl = $scheme . "://" . $host . $parsedUrl['path'] . ($newQuery ? '?' . $newQuery : '');
    ...
    public function category($category_slug = '')
    {
        ...
        if (!is_numeric($page_no) || $page_no < 1) {
            redirect(base_url('products/category/' . $category_slug));
        }
        ...
        $user_id = $this->data['is_logged_in'] ? $this->data['user']->id : null;
        ...
        // Fix "Next" link for page 1
        if ($page_no == 1 && empty($this->uri->segment(4))) {
            $config['cur_page'] = 1; // Explicitly set current page to 1
        } else {
            $config['cur_page'] = $page_no; // Use the page number from URL
        }
        ...
        // Post-process pagination links to fix "Next" link on page 1
        if ($page_no == 1 && empty($this->uri->segment(4))) {
            $next_page_url = base_url('products/category/' . $category_slug . '/2');
            if ($this->input->get()) {
                $next_page_url .= '?' . http_build_query($this->input->get());
            }
            $pagination_links = preg_replace(
                '/<a class="page-link" href="[^"]*\/1(\?.*?)?">(<i class="fa fa-arrow-right"><\/i>)<\/a>/',
                '<a class="page-link" href="' . $next_page_url . '">$2</a>',
                $pagination_links
            );
        }

        $this->data['links'] = $pagination_links;
