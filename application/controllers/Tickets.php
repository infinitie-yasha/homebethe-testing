...
class Tickets extends CI_Controller
{
    ...
    public function update_ticket()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $ticket_id = $this->input->post('edit_id', true);
            $user_id = $this->input->post('user_id', true);
    ...
    public function send_message()
    {
        if ($this->ion_auth->logged_in()) {
            ...
            if (!empty($_FILES) && isset($_FILES['attachments']['name'][0]) && !empty($_FILES['attachments']['name'][0])) {
                ...
                if (!empty($insert_id)) {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($user_roles as $user) {
                            // $user_res = fetch_details('users', ['id' => $user['user_id']],  'fcm_id,platform_type');
                            $user_res = fetch_details('user_fcm', ['user_id' => $user['user_id']],  'fcm_id,platform_type');
                            $groupedByPlatform = [];
    ...
    public function get_ticket_list()
    {
        if ($this->ion_auth->logged_in()) {
            ...
            $ticket_data =  $this->ticket_model->get_user_ticket_list($user_id, $search);
            return $ticket_data;
        } else {
