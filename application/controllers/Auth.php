    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if (!empty($res)) {
                ...
                if ($_POST['type_otp'] ) {
                    $identity = $this->input->post('identity', true);
                    $user = $this->ion_auth->where('mobile', $identity)->users()->row();
                    ...
                    $seller_data = $this->db->select('status')->where('user_id', $user->id)->get('seller_data')->row_array();
                    if (!empty($seller_data)) {
                        if ($seller_data['status'] == 2) {
                            $response['error'] = true;
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                            $response['message'] = 'Your seller account is suspended. Please contact administrator.';
                            echo json_encode($response);
                            return;
                        } elseif ($seller_data['status'] == 0) {
                            $response['error'] = true;
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                            $response['message'] = 'Your seller account is not approved yet. Please contact administrator.';
                            echo json_encode($response);
                            return;
                        }
                    } else {

                        if (!empty($res) && $res[0]['active'] == 0) {
                            $response['error'] = true;
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                            $response['message'] = 'Your account is deactivated.Please contact administrator.';
                            echo json_encode($response);
                            return;
                        }
                    }

                    // Mimic login: Set required session keys
                    $this->session->set_userdata([
                        ...
                        'ion_auth_session_hash' => $this->config->item('session_hash', 'ion_auth'), // ✅ FIXED
                    ]);
                    ...
                    // Then proceed as if login was successful
                    $seller_status = $this->ion_auth->seller_status($user->id);
                    $messages = array(
                    ...
                    if (!$this->input->is_ajax_request() && ($seller_status == '0')) {
                ...
                if ($this->ion_auth_model->in_group('admin', $res[0]['id'])) {
                    ...
                    if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember, 'phone')) {
                        //if the login is successful
                        $delivery_boy_status = $this->ion_auth->delivery_boy_status($this->session->userdata('user_id'));
    ...
    public function verify_user()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            // Defensive: if mobile not found, return explicit message before role checks
