...
class Invoice_model extends CI_Model
{
    ...
    public function get_seller_sales_list(
        ...
        $search_res = $this->db->select(' o.*,oitems.* , u.username ,u.email,u.mobile,u.image as user_profile , sd.store_name,us.username as seller_name ')
            ->join('users u', 'u.id= o.user_id', 'left')
            ->join('order_items oitems', 'o.id=oitems.order_id', 'left');
        if (!empty($_GET['seller_id']) || !empty($_POST['seller_id'])) {
        ...
        foreach ($user_details as $row) {
            ...
            if (empty($row['user_profile'])) {
                $row['profile'] = base_url() . NO_IMAGE;
            } else {
                $row['profile'] = base_url() . $row['user_profile'];
            }
            $tempRow['id'] = $row['id'];
            $tempRow['name'] = ($row['username'] != '' || $row['username'] != null) ? $row['username'] : '';
            $tempRow['user_profile'] = $row['profile'];
        ...
        $bulkData['total'] = $total;
        $bulkData['grand_total'] =  (string)$total_amount;
        $bulkData['total_delivery_charge'] = (string)$total_delivery_charge;
        $bulkData['grand_final_total'] =   (string)$final_total_amount;
        $bulkData['rows'] = $rows;
