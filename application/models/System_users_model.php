...
class System_users_model extends CI_Model
{
    ...
    function update_user($data)
    {
        ...
        if (isset($data['edit_system_user']) && !empty($data['edit_system_user'])) {
    ...
    function get_users_list()
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['up.`id`' => $search, '`u.username`' => $search, 'u.`mobile`' => $search, 'u.`email`' => $search];
        }
        ...
        foreach ($sys_search_res as $row) {
            ...
            if ($current_user_id != $row['user_id'] && $userData[0]['role'] == 0) {
                ...
                // Edit System User
                $operate .= '<li>
                    <a class="dropdown-item edit_system_btn" href="javascript:void(0)" 
                       data-id="' . $row['id'] . '" 
                       data-url="admin/system_users/add_system_users">
                ...
                if ($row['active'] == '1') {
                    $tempRow['status'] = '<a class="badge bg-success-lt">Active</a>';
                    $operate .= '<li>
                        <a class="dropdown-item update_active_status" href="javascript:void(0)" 
                           data-table="users" 
                           data-id="' . $row['user_id'] . '" 
                           data-status="' . $row['active'] . '">
                            <i class="ti ti-eye me-2"></i>Deactivate
                        </a>
                    </li>';
                } else {
                    $tempRow['active'] = '<a class="badge bg-danger-lt">Inactive</a>';
                    $operate .= '<li>
                        <a class="dropdown-item update_active_status" href="javascript:void(0)" 
                           data-table="users" 
                           data-id="' . $row['user_id'] . '" 
                ...
                $operate .= '<li>
                    <a class="dropdown-item text-danger" href="javascript:void(0)"
                       x-data="ajaxDelete({
                           ...
                           id: \'' . $row['user_id'] . '\',
                           tableSelector: \'#system_users_table\',
                           confirmTitle: \'Delete System User\',
                           confirmMessage: \'Do you really want to delete this system user?\'
                       })"
                       @click="deleteItem">
            ...
            if ($row['role'] == '0') {
                $row['role'] = "<span class='badge bg-primary-lt'>Super Admin</span>";
            }
            if ($row['role'] == '1') {
                $row['role'] = "<span class='badge bg-danger-lt '>Admin</span>";
            }
            if ($row['role'] == '2') {
                $row['role'] = "<span class='badge bg-warning-lt'>Editor</span>";
            }
            if ($row['role'] == '3') {
                $row['role'] = "<span class='badge bg-secondary-lt'>Supporter</span>";
            }
