    ...
    function get_user_cart($user_id, $is_saved_for_later = 0, $product_variant_id = '')
    {
        ...
        if (!empty($res)) {
            $res = array_map(function ($d) {
                ...
                if ($d['type'] != 'simple_product') {
                    $stock = $d['product_variant_stock'];
                    $d['stock'] = $stock;
                    unset($d['product_variant_stock']);
                }
