...
class Rating_model extends CI_Model
{
    ...
    public function get_rating($seller_id = '')
    {
        $offset = 0;
        $limit = 10;
        $sort = 'id';
        ...
        $rating_count = $count_res->get('product_rating pr')->row_array();

        
        $total = $rating_count['total'] ?? 0;

        // Fetch ratings
        $search_res = $this->db->select('pr.*, u.username as user_name, p.name as product_name')
            ->join('users u', 'u.id=pr.user_id')
            ->join('products p', 'p.id=pr.product_id');
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end table-dropdown-menu">
                <li>
                    <a class="dropdown-item text-danger" href="javascript:void(0)"
                       x-data="ajaxDelete({
                           url: base_url + \'admin/product_ratings/delete_rating\',
                           id: \'' . $row['id'] . '\',
                           tableSelector: \'#products_ratings_table\',
                           confirmTitle: \'Delete Product Rating\',
                           confirmMessage: \'Do you really want to delete this product rating?\'
                       })"
                       @click="deleteItem">
            ...
            if (!empty($row['images'])) {
                ...
                for ($j = 0; $j < count($images); $j++) {
                    ...
                    if ($j == 0) {
                        ...
                        $tempRow['images'] = '<div class="row"><div class="col-md-6"><div class="mx-auto image-box-table"><a href="' . $image_url . '" data-toggle="lightbox" data-gallery="' . $image_unique_name . '"> <img src="' . $image_url . '" class="img-fluid rounded"> </a></div></div><div class="col-md-6 my-auto "> <span class="text-primary"> ' . $counter . '</span></div></div>';
                    } else {
                        $tempRow['images'] .= '<div class="mx-auto image-box-table d-none"><a href="' . $image_url . '" data-toggle="lightbox" data-gallery="' . $image_unique_name . '"><img src="' . $image_url . '" class="img-fluid rounded"></a></div>';
                    }
                }
            } else {
                $tempRow['images'] = '-';
            ...
            $tempRow['rating'] = '<div class="rating">' . $stars . '</div>';
