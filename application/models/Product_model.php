...
class Product_model extends CI_Model
{
    ...
    public function add_product($data)
    {
        ...
        $deletedAttributeValueIds = [];
        $editable_attribute_value_ids = [];
        $attributeValueString = '';
        if (!empty($data['attribute_values'])) {
            $editable_attribute_value_ids = explode(',', $data['attribute_values']);
        }


        if (!empty($data['edit_variant_id'])) {

            $editable_variants = $this->db
                ->select('id, attribute_value_ids')
                ->where('product_id', $data['edit_product_id'])
                ->where('status !=', 0)
                ->get('product_variants')
                ->result_array();


            $remainingVariantIds = $data['edit_variant_id'];
            $remainingAttributeIds = [];
            foreach ($editable_variants as $variant) {
                if (in_array($variant['id'], $remainingVariantIds) && !empty($variant['attribute_value_ids'])) {
                    $remainingAttributeIds = array_merge(
                        $remainingAttributeIds,
                        explode(',', $variant['attribute_value_ids'])
                    );
                }
            }

            $remainingAttributeIds = array_values(
                array_unique(array_filter($remainingAttributeIds))
            );

            $attributeValueString = implode(',', $remainingAttributeIds);


            // merge variant ids
            $data['edit_variant_id'] = array_values(
                array_unique(
                    array_merge(
                        $data['edit_variant_id'],
                        array_column($editable_variants, 'id')
                    )
                )
            );
        ...
        $approval_status = (isset($data['approval_status'])) ? $data['approval_status'] : 'pending';
        $product_rejection_reason = (isset($data['product_rejection_reason'])) ? $data['product_rejection_reason'] : null;
        ...
        if (isset($data['edit_product_id']) && !empty($data['edit_product_id'])) {
            ...
            $status = 0;
            $status = (isset($s_permits['require_products_approval']) && $s_permits['require_products_approval'] == 0) ? 1 : 2;
            if ($approval_status == 'approved') {
                $status = 1;
            } else {
                $status = 0;
            }
            $require_products_approval = $status;
        }
        ...
        $pro_data = [
            ...
            'rejection_reason' => $product_rejection_reason,
        ];
        ...
        $pro_attr_data = [
            'product_id' => $p_id,
            'attribute_value_ids' => !empty($attributeValueString) ? $attributeValueString : strval($data['attribute_values']),
        ];
    ...
    public function get_product_details($flag = NULL, $seller_id = NULL, $p_status = NULL, $from_faq = 0, $from_select = 0)
    {
        ...
        $search_res = $this->db->select('product_variants.id AS id,c.name as category_name,b.name as brand_name,sd.store_name, p.id as pid,p.rating,p.no_of_ratings,p.name, p.type, p.image, p.status, p.approval_status, p.rejection_reason  ,p.brand,product_variants.price , product_variants.special_price, product_variants.stock')
            ->join("categories c", "p.category_id=c.id")
            ->join("brands b", "p.brand=b.id", 'left')
        ...
        foreach ($pro_search_res as $row) {
            ...
            switch ($row['approval_status']) {
                case 'pending':
                    $approval_status_label = '<a class="badge bg-warning-lt text-white">Pending</a>';
                    break;
                case 'approved':
                    $approval_status_label = '<a class="badge bg-success-lt text-white">Approved</a>';
                    break;
                case 'rejected':
                    $approval_status_label = '<a class="badge bg-danger-lt text-white">Rejected</a>';
                    break;

                default:
                    $approval_status_label = '<a class="badge bg-secondary-lt text-dark">' . ucfirst($row['approval_status'] == null ? 'Unknown' : $row['approval_status']) . '</a>';
                    break;
            }
            ...
            $tempRow['approval_status'] = $approval_status_label;
            $tempRow['rejection_reason'] = $row['rejection_reason'];
            $variations = '';
