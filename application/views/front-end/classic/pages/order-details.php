                ...
                <div class="mb-4  border-0">
                                ...
                                <h6 class="ml-auto mr-3">
                                    ...
                                    if ($order['order_items'][0]['is_already_cancelled'] != 1) { ?>
                                        ...
                                        <a target="_blank"
                                            ...
                                            class='button button-primary-outline'><?= !empty($this->lang->line('invoice')) ? str_replace('\\', '', $this->lang->line('invoice')) : 'Invoice' ?></a>
                                    <?php }
                                    ?>
                                    <a href="<?= base_url('my-account/orders/') ?>"
                                        class='button button-danger-outline'><?= !empty($this->lang->line('back_to_list')) ? str_replace('\\', '', $this->lang->line('back_to_list')) : 'Back to List' ?></a>
                                </h6>
                            </div>
                        </div>
                        <br>
                                ...
                                <form class="form-horizontal " id="send_bank_receipt_form"
                                    ...
                                    <div class="form-group ">
                                        <label for="receipt"> <strong>
                                                <?= !empty($this->lang->line('bank_payment_receipt')) ? str_replace('\\', '', $this->lang->line('bank_payment_receipt')) : 'Bank Payment Receipt' ?></strong>
                                        </label>
                                    ...
                                    <div class="form-group">
                                        <button type="reset"
                                            class="button button-warning-outline"><?= !empty($this->lang->line('reset')) ? str_replace('\\', '', $this->lang->line('reset')) : 'Reset' ?></button>
                                        <button type="submit" class="button button-success-outline"
                                            id="submit_btn"><?= !empty($this->lang->line('send')) ? str_replace('\\', '', $this->lang->line('send')) : 'Send' ?></button>
                                    </div>
                                </form>
                                ...
                                <div class="col-md-6">
                                    ...
                                    foreach ($bank_transfer as $row1) { ?>
                                        <small>[<a href="<?= base_url() . $row1['attachments'] ?>"
                                                target="_blank"><?= !empty($this->lang->line('attachment')) ? str_replace('\\', '', $this->lang->line('attachment')) : 'Attachment' ?>
                                                <?= $i ?> </a>]</small>
                                        <?php if ($row1['status'] == 0) { ?>
                                            <label
                                                class="badge badge-warning"><?= !empty($this->lang->line('pending')) ? str_replace('\\', '', $this->lang->line('pending')) : 'Pending' ?></label>
                                        <?php } else if ($row1['status'] == 1) { ?>
                                                <label
                                                    class="badge badge-danger"><?= !empty($this->lang->line('rejected')) ? str_replace('\\', '', $this->lang->line('rejected')) : 'Rejected' ?></label>
                                        <?php } else if ($row1['status'] == 2) { ?>
                                                    <label
                                                        class="badge badge-primary"><?= !empty($this->lang->line('accepted')) ? str_replace('\\', '', $this->lang->line('accepted')) : 'Accepted' ?></label>
                                        <?php } else { ?>
                                                    <label
                                                        class="badge badge-danger"><?= !empty($this->lang->line('invalid_value')) ? str_replace('\\', '', $this->lang->line('invalid_value')) : 'Invalid Value' ?></label>
                                        <?php } ?>
                                        <?php $i++;
                                    } ?>
                                </div>
                    ...
                    <div class="card-body">
                                    ...
                                    if (!empty($item['url']) || !empty($item['shiprocket_order_tracking_url'])) { ?>
                                        ...
                                        <?php if (isset($item['courier_agency']) && !empty($item['courier_agency'])) { ?>
                                            <p> <span class="text-muted">
                                                    <?= !empty($this->lang->line('courier_agency')) ? str_replace('\\', '', $this->lang->line('courier_agency')) : 'Courier Agency' ?>
                                                    : </span><a href="<?= $item['url'] ?>"
                                                    title="<?= !empty($this->lang->line('click_here_to_trace_the_order')) ? str_replace('\\', '', $this->lang->line('click_here_to_trace_the_order')) : 'click here to trace the order' ?>"><?= $item['courier_agency'] ?></a>
                                            </p>
                                    ...
                                    <?php if ($item['type'] == 'digital_product' && $item['download_allowed'] == 1 && ($item['active_status'] == 'received' || $item['active_status'] == 'delivered')) {
                                        ...
                                        <div class="media-body mt-3">
                                            <a href="<?= base_url('products/download_link_hash/' . $item['id']) ?>"
                                                title="<?= !empty($this->lang->line('download')) ? str_replace('\\', '', $this->lang->line('download')) : 'Download' ?>"
                                                class="btn btn-outline-info"><i class="fas fa-download"></i>
                                                <?= !empty($this->lang->line('download')) ? str_replace('\\', '', $this->lang->line('download')) : 'Download' ?></a>
                                        </div>
                                    <?php }
                                    if ($item['type'] == 'digital_product' && $item['download_allowed'] == 0) { ?>
                                        <div class="media-body mt-3">
                                            <span
                                                class="text-danger"><?= !empty($this->lang->line('you_will_receive_this_item_from_seller_via_email')) ? str_replace('\\', '', $this->lang->line('you_will_receive_this_item_from_seller_via_email')) : 'You will receive this item from seller via email' ?></span>
                                        </div>
                                    <?php } ?>
                                </div>
                                <div class="align-self-center img-fluid">
                                    <a href="<?= base_url('products/details/' . $item['slug']) ?>"><img
                                            src="<?= $item['image_sm'] ?>" width="180 " height="180"></a>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <h6 class="h5">
                                    <?= !empty($this->lang->line('shipping_details')) ? str_replace('\\', '', $this->lang->line('shipping_details')) : 'Shipping Details' ?>
                                </h6>
                                <hr>
                            ...
                            <div class="col-md-6">
                                <h6 class="h5">
                                    <?= !empty($this->lang->line('price_details')) ? str_replace('\\', '', $this->lang->line('price_details')) : 'Price Details' ?>
                                </h6>
                                <hr>
                                <div class="table-responsive">
                                    <table class="table table-borderless">
                                        <tbody>
                                            <tr>
                                                <th><?= !empty($this->lang->line('total_order_price')) ? str_replace('\\', '', $this->lang->line('total_order_price')) : 'Total Order Price' ?>
                                                </th>
                                                <td>+
                                            ...
                                            <?php if ($item['type'] != 'digital_product') { ?>
                                                <tr>
                                                    <th><?= !empty($this->lang->line('delivery_charge')) ? str_replace('\\', '', $this->lang->line('delivery_charge')) : 'Delivery Charge' ?>
                                                    </th>
                                                    <td>+
                                            ...
                                            <?php } ?>
                                            <tr class="d-none">
                                                <th><?= !empty($this->lang->line('tax')) ? str_replace('\\', '', $this->lang->line('tax')) : 'Tax' ?>
                                                    - (<?= $order['total_tax_percent'] ?>%)</th>
                                                <td>+
                                            ...
                                            <?php if (!empty($order['promo_code']) && !empty($order['promo_discount'])) { ?>
                                                <tr>
                                                    <th><?= !empty($this->lang->line('promocode_discount')) ? str_replace('\\', '', $this->lang->line('promocode_discount')) : 'Promocode Discount' ?>
                                                        - (<?= $order['promo_code'] ?>)
                                                    </th>
                                                    <td>-
                                            ...
                                            <?php } ?>
                                            <tr>
                                                <th><?= !empty($this->lang->line('wallet_used')) ? str_replace('\\', '', $this->lang->line('wallet_used')) : 'Wallet Used' ?>
                                                </th>
                                                <td>-
                                            ...
                                            <tr class="h6">
                                                <th><?= !empty($this->lang->line('final_total')) ? str_replace('\\', '', $this->lang->line('final_total')) : 'Final Total' ?>
                                                </th>
                                                <td><?= $settings['currency'] . ' ' . number_format($order['total_payable'], 2) ?><span
                                                        class="small text-muted">
                                                        <?= !empty($this->lang->line('via')) ? str_replace('\\', '', $this->lang->line('via')) : 'via' ?>
                                                        (<?= $order['payment_method'] ?>) </span></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            if (!$item['is_already_cancelled'] && $item['product_is_cancelable'] && $active_index <= $cancellable_index) { ?>
                                <div class="col my-auto">
                                    <a class="update-order block button-sm buttons btn-6-1 mt-3 m-0" data-status="cancelled"
                                        data-order-id="<?= $order['id'] ?>"><?= !empty($this->lang->line('cancel')) ? str_replace('\\', '', $this->lang->line('cancel')) : 'Cancel' ?></a>
                                </div>
                            <?php } ?>
                            <?php
                            $order_date = $order['order_items'][0]['status'][3][1];
                    ...
                    <div id="modal-custom" class="returnModal" data-iziModal-group="grupo1">
                        ...
                        <div class="modal-body py-0">
                            <div class="d-flex flex-column flex-wrap gap-2 mb-3">
                                ...
                                <input type="text" id="otherReasonField" class="form-control mt-2"
                                    placeholder="Enter your reason" style="display: none;">
                                <!-- Image Upload Section -->
                                <div class="mb-3">
                                    <label for="returnImage" class="form-label">Upload Image of Item</label>
                                    <input type="file" class="form-control" id="return_item_image"
                                        name="return_item_images[]" accept="image/*" multiple>
                                </div>
                            </div>
