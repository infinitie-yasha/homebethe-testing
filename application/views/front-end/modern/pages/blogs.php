            ...
            <nav class="d-inline-block" aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 bg-transparent">
                    ...
                    <li class="breadcrumb-item active text-muted" aria-current="page">
                        <?= !empty($this->lang->line('blogs')) ? str_replace('\\', '', $this->lang->line('blogs')) : 'Blogs' ?>
                    </li>
                </ol>
            </nav>
            <!-- /nav -->
    ...
    <div class="container mb-15">
                ...
                <div class="ele-wrapper d-flex align-items-center flex-wrap">
                    <div class="form-group col-md-5 pl-0 blog_category">
                        ...
                        <select class='form-control' name='category_parent' id="category_parent">
                            <option value="">
                                <?= !empty($this->lang->line('select_category')) ? str_replace('\\', '', $this->lang->line('select_category')) : 'Select Category' ?>
                            </option>
                            <?php foreach ($fetched_data as $categories) { ?>
                                <option value="<?= $categories['id'] ?>"
                                    <?= ($this->input->get('category_id') == $categories['id']) ? 'selected' : '' ?>>
                                    <?= $categories['name'] ?>
                                </option>
                            <?php } ?>
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="search" name="blog_search" class="form-control" id="blog_search"
                            value="<?= (isset($blog_search) && !empty($blog_search)) ? $blog_search : "" ?>"
                            placeholder="<?= !empty($this->lang->line('search_blog')) ? str_replace('\\', '', $this->lang->line('search_blog')) : 'Search Blog' ?>">
                    </div>
                    <div class="dropdown float-md-right mb-4 col-md-3 form-select-wrapper">
                        <div class="d-flex align-items-baseline mt-5">
                            <label class="mr-2 dropdown-label fw-bold fs-16">
                                <?= !empty($this->lang->line('show')) ? str_replace('\\', '', $this->lang->line('show')) : 'Show' ?>:</label>
                            <a class="dropdown-border form-select col-6" data-toggle="dropdown" role="button"
                                aria-haspopup="true"
        ...
        <div class="row">
            ...
            <?php if (!isset($blogs['data']) && empty($blogs['data']) || $blogs['data'] == []) { ?>
                <div>
                    <h1 class="h2 text-center">
                        <?= !empty($this->lang->line('no_blogs_found')) ? str_replace('\\', '', $this->lang->line('no_blogs_found')) : 'No Blogs Added Yet.' ?>
                    </h1>
                </div>
            ...
            <?php foreach ($blogs['data'] as $row) {
                $image = $row['image'];
                $encodedImage = dirname($image) . '/' . rawurlencode(basename($image));
            ?>
                <div class="item-inner col-md-4">
                    <article>
                        <div class="card">
                            <figure class="card-img-top overlay overlay-1 hover-scale">
                                <a href="<?= base_url("blogs/view_detail/" . $row['slug']) ?>"> <img
                                        src="<?= base_url($encodedImage) ?>" alt=""
                                        class="image-fit" /></a>
                                <figcaption>
                            ...
                            <div class="card-body p-4 overflow-hidden" style="height: 200px;">
                                <div class="post-header">
                                    <h2 class="post-title h3 mt-1 mb-3"><a class="link-dark text-decoration-none"
                                            href="<?= base_url("blogs/view_detail/" . $row['slug']) ?>">
