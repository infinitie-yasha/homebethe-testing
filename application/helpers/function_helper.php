...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $is_detailed_data = 0)
{
    ...
    if (isset($settings['single_seller_system']) && $settings['single_seller_system'] == '1') {
        $seller_id = $settings['single_seller_system_seller_id'];
    }
    ...
    if (isset($filter['show_only_stock_product']) && ($filter['show_only_stock_product'] == 1 || $filter['show_only_stock_product'] == true)) {
    ...
    if (!empty($filter['search'])) {
        ...
        $t->db->group_start();
        $t->db->like('p.name', $search);
        $t->db->or_like('p.slug', $search);
        $t->db->or_like('p.description', $search);
        $t->db->or_like('p.tags', $search);
        $t->db->group_end();
    }
    ...
    if (isset($filter) && $filter != null) {
        if (!empty($attr_value_ids)) {
            $json_encode_data = get_attribute_values_by_id($attr_value_ids);
            $response['filters'] = json_decode($json_encode_data, true);
        }
    } else {
        $response['filters'] = [];
    }
...
function update_wallet_balance($operation, $user_id, $amount, $message = "Balance Debited", $order_item_id = "", $is_refund = 0, $transaction_type = 'wallet', $status = '')
{
    ...
    // print_R($operation == 'debit' && $amount > $user_balance[0]['balance']);
    // die();
...
function get_attribute_values_by_pid($id)
{
    ...
    if (!empty($attribute_values) && !empty($attribute_values[0]['ids'])) {
    ...
    $attribute_values[0]['ids'] = $attribute_values[0]['ids'] ?? '';
    $attribute_values[0]['value'] = $attribute_values[0]['value'] ?? '';
    $attribute_values[0]['attr_name'] = $attribute_values[0]['attr_name'] ?? '';
    $attribute_values[0]['name'] = $attribute_values[0]['name'] ?? '';
    $attribute_values[0]['swatche_type'] = $attribute_values[0]['swatche_type'] ?? '';
    $attribute_values[0]['swatche_value'] = $attribute_values[0]['swatche_value'] ?? '';
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '', $delivery_method = '', $token = '')
{
    ...
    if (
        !empty($system_settings['single_seller_system'])
        && $system_settings['single_seller_system'] == 1
        && !empty($system_settings['single_seller_system_seller_id'])
    ) {

        $t->db->where('p.seller_id', $system_settings['single_seller_system_seller_id']);
    }
...
function fetch_orders($order_id = NULL, $user_id = NULL, $multi_status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null, $order_type = '', $from_seller = false, $draftFilter = 1)
{
    ...
    if (isset($multi_status) && is_array($multi_status) && count($multi_status) > 0) {
        $multi_status = array_map('trim', $multi_status);
        $count_res->where_in('oi.active_status', $multi_status);
    }
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        if (!empty($multi_status)) {
            $subquery = $t->db->select('order_items.order_id', false) // false to prevent escaping
                ->from('order_items')
                ->where_in('active_status', $multi_status)
                ->get_compiled_select();
        }
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                $order_item_data[$k]['attachment'] = (empty($order_item_data[$k]['attachment']) || file_exists(FCPATH . $order_item_data[$k]['attachment']) == FALSE) ? '' : get_image_url($order_item_data[$k]['attachment']);
        ...
        for ($i = 0; $i < $cart[0]['cart_count']; $i++) {
            ...
            if (isset($settings['shiprocket_shipping_method']) && $settings['shiprocket_shipping_method'] == 1) {
                if (!$tmpRow['is_deliverable'] && $cart[$i]['pickup_location'] != "") {
                    ...
                    $pickup_pincode = fetch_details('pickup_locations', ['id' => $cart[$i]['pickup_location']], 'pin_code');
...
function check_parcels_deliveriblity($parcels, $user_pincode)
{
    ...
    foreach ($parcels as $seller_id => $parcel) {
        foreach ($parcel as $pickup_location => $parcel_weight) {
            ...
            $pickup_postcode = fetch_details('pickup_locations', ['id' => $pickup_location], 'pin_code');
...
function sendWebJsonResponse(bool $error, string $message = "", array $additional_data = [], array $data = [], int $status_code = 200): void
{
    // Get CI instance
    $CI = &get_instance();

    // Add CSRF tokens only for POST
    $csrf = [];
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $csrf['csrfName'] = $CI->security->get_csrf_token_name();
        $csrf['csrfHash'] = $CI->security->get_csrf_hash();
    }
...
function getFriendlyTableName($table)
{
    $names = [
        'users' => 'User',
        'categories' => 'Category',
    ...
    return isset($names[$table]) ? $names[$table] : ucfirst(str_replace('_', ' ', $table));
}
if (!function_exists('getTopParentCategory')) {

    /**
     * Get the top-most parent category using ONLY ONE database query.
     *
     * WHY RAW SQL?
     * -----------------------------------------------------------------
     * CodeIgniter 3's Query Builder does NOT support Recursive CTEs
     * (Common Table Expressions). A recursive CTE lets MySQL walk a
     * hierarchical structure (parent → child → grandchild), WITHOUT
     * requiring multiple PHP calls or multiple SQL queries.
     *
     * By using a recursive CTE:
     *   ✔ MySQL resolves the entire parent chain internally
     *   ✔ Only ONE SQL query is executed
     *   ✔ Fast even for deeply nested categories (unlimited depth)
     *   ✔ No PHP recursion, no DB loops, no repeated SELECT
     *
     * REQUIREMENTS:
     * -----------------------------------------------------------------
     * - MySQL 8+ (recursive CTE was introduced in MySQL 8)
     *
     * @param int $id - Category ID for which you want the root parent
     * @return object|null - Returns the top-most parent category record
     */
    function getTopParentCategory($id)
    {
        // Access CI Superobject
        $CI =& get_instance();

        $sql = "
            WITH RECURSIVE category_path AS (
                -- Start with the provided category ID
                SELECT id, name, parent_id , commission_perc
                FROM categories
                WHERE id = ?

                UNION ALL

                -- Move upward by joining each parent
                SELECT c.id, c.name, c.parent_id , c.commission_perc
                FROM categories c
                INNER JOIN category_path cp ON c.id = cp.parent_id
            )
            -- The root category is the one with parent_id = 0 or NULL
            SELECT *
            FROM category_path
        ...
        return $CI->db->query($sql, [$id])->row();
    }
}

function get_available_payment_gateways(array $settings): array
{
    $map = [
        'razorpay' => [
            ...
            [
                'razorpay_key_id',
                'razorpay_secret_key',
                'refund_webhook_secret_key' => 'razorpay_refund_webhook_secret_key'
            ]
        ],
        'phonepe' => [
...
/**
 * Generate a fast-loading optimized WebP image URL.
 *
 * This helper function:
 * - Accepts an image URL (JPEG / PNG)
 * - Converts it into a WebP image (one-time only)
 * - Stores the optimized image in `/optimized/webp/` directory
 * - Returns the cached WebP URL for faster page load
 *
 * Features:
 * - Automatic caching (no reprocessing if already optimized)
 * - No external libraries required (uses PHP GD)
 * - Preserves transparency for PNG images
 * - Safe fallback to original image if conversion fails
 *
 * Requirements:
 * - PHP GD extension enabled
 * - WebP support enabled in GD
 *
 * Usage:
 *   echo fast_image_url(base_url('uploads/product/image.jpg'));
 *
 * @param string $image_url  Full image URL (JPEG / PNG)
 * @param int    $quality    WebP image quality (1–100), default 70
 *
 * @return string Optimized WebP image URL or original URL on failure
 */
function fast_image_url($image_url, $quality = 70)
{
    ...
    // Get CodeIgniter instance (for consistency in helpers)
    $CI =& get_instance();

    // Get absolute document root path
    $doc_root = rtrim(FCPATH, '/');

    // Parse URL to extract file path
    $parsed_url = parse_url($image_url);

    // If path is missing, return original URL
    if (!isset($parsed_url['path'])) {
        return $image_url;
    }

    // Convert URL path to server file path
    $original_path = $doc_root . $parsed_url['path'];

    // If original image does not exist, return original URL
    if (!file_exists($original_path)) {
        return $image_url;
    }

    // Create optimized WebP directory
    $optimized_dir = dirname($original_path) . '/optimized/webp/';
    if (!is_dir($optimized_dir)) {
        mkdir($optimized_dir, 0777, true);
    }

    // Generate optimized WebP filename
    $filename = pathinfo($original_path, PATHINFO_FILENAME);
    $optimized_path = $optimized_dir . $filename . '.webp';

    // Generate optimized WebP URL
    $optimized_url = str_replace(
        $parsed_url['path'],
        dirname($parsed_url['path']) . '/optimized/webp/' . $filename . '.webp',
        $image_url
    );

    // If optimized image already exists, return cached version
    if (file_exists($optimized_path)) {
        return $optimized_url;
    }

    // Get image information (MIME type, dimensions, etc.)
    $info = getimagesize($original_path);

    // If image is invalid, return original URL
    if (!$info) {
        return $image_url;
    }
    ...
    imagewebp($image, $optimized_path, $quality);

    // Free memory
    imagedestroy($image);

    // Return optimized WebP image URL
    return $optimized_url;
}



function normalize_variant_ids($ids)
{
    $parts = explode(',', $ids);
