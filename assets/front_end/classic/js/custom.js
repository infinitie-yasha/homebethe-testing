...
const max_allow_items_in_cart = $('#allow_items_in_cart').val();
const is_cart_single_seller_enabled = $('#is_cart_single_seller_enabled').val() == '1' ? true : false;
...
$(function () {
        ...
        $("#quick-view").iziModal({
            ...
            onOpening: function (modal) {
                ...
                $.getJSON(base_url + 'products/get-details/' + modal.$element.data('dataProductId'), function (data) {
                    ...
                    $('#modal-add-to-cart-button').attr('data-seller-id', data.seller_id);
                    $('#modal-add-to-cart-button').attr('data-product-id', data.id);
                    $('#modal-buy-now-button').attr('data-product-id', data.id);
                    $('#modal-buy-now-button').attr('data-seller-id', data.seller_id);
                        ...
                        $.each(attribute_ids, function (j, id) {
                            ...
                            if (swatche_types[j] == "1") {
                                ...
                                variant_attributes += '<style> .product-page-details .btn-group>.active { border: 1px solid black;}</style>' + '<button class="btn fullCircle" ' + color_code + '>' +
                                    ...
                                    '<input type="radio" data-name="' + color_code + '" name="' + e.attr_name + '" value="' + id + '" class="modal-product-attributes" autocomplete="off"><br>' +
                                ...
                                variant_attributes += style1 + '<label class="btn btn-default text-center">' +
                                    ...
                                    '<input type="radio"  data-name="' + attribute_values[j] + '" name="' + e.attr_name + '" value="' + id + '" class="modal-product-attributes" autocomplete="off">' + attribute_values[j] + '<br>' +
                    ...
                    if (!$.isEmptyObject(data.tags)) {
                        ...
                        $.each(data.tags, function (i, e) {
                            tags += '<a  style="max-height: 100px; overflow-y: auto; display: flex; flex-wrap: wrap; gap: 0.25rem;" href="' + base_url + 'products/tags/' + e + '" target="_blank"><span class="badge badge-secondary p-1">' + e + '</span></a>';
                        });
    ...
    $('#modal-add-to-cart-button').on('click', function (e) {
        ...
        $('.modal-product-attributes:checked').each(function () {
            selectedAttributesNames.push($(this).data('name'));
        });

        let product_seller_id = $(this).data('seller-id') ?? null;
        ...
        if (is_loggedin == 0) {
            ...
            var cart_item = {
                ...
                "min": min,
                "step": step,
                'attribute_value_names': selectedAttributesNames,
            ...
            if (cart !== null && cart !== undefined && cart.length + 1 > allow_items_in_cart) {
                Toast.fire({
                    icon: 'error',
                ...
                if (cart !== null && cart !== undefined) {
                    if (is_cart_single_seller_enabled) {
                        var newSellerId = product_seller_id; // seller of product being added
                        var hasDifferentSeller = cart.some(function (item) {
                            return (item.product_seller_id) !== (newSellerId);
                        });
                        if (hasDifferentSeller) {
                            Toast.fire({
                                icon: 'error',
                                title: 'Only single seller items are allow in cart.You can remove privious item(s) and add this item.'
                            });
                            return;
                        }
                    }
                    ...
                    if (cartItem) {
                        ...
                        cart = updateCartItem(cart, product_variant_id, update_qty, update_qty * price);
...
function updateCartItem(cart, variantId, newQty, newPrice) {
    return cart.map(item => {
        if (item.product_variant_id === String(variantId)) {
            return {
                ...item,
                qty: String(newQty),
    ...
    $(document).on('change', '.product-quantity input,.product-sm-quantity input,.itemQty, .header_qty', function (e) {
        ...
        if (qty % step == 0) {
            ...
            if (is_loggedin == 1) {
                ...
                if (cart.length) {
                    cart = updateCartItem(cart, id, qty, qty * price);
...
$(document).on('click', '.add_to_cart', function (e) {
    ...
    if (izi_modal == "" || izi_modal == undefined) {
        ...
        if (is_loggedin == 0) {
            ...
            if (cart !== null && cart !== undefined) {
                if (cart.length + 1 > max_allow_items_in_cart) {
                    Toast.fire({
                        icon: 'error',
                        title: `Maximum ${max_allow_items_in_cart} Item(s) Can Be Added Only!`
                    });
                    $('[name="qty"]').val(qty - 1);
                    return;
                }
                ...
                if (cartItem) {
                    let update_qty = Number(cartItem.qty) + Number(qty);
                    console.log('update_qty = ' + update_qty);
                    cart = updateCartItem(cart, product_variant_id, update_qty, update_qty * price);
                    localStorage.setItem("cart", JSON.stringify(cart));
...
function display_cart(cart) {
    ...
    if (cart !== null && cart.length > 0) {
        ...
        cart.forEach((item) => {
            ...
            let attribute_value_names = [];
            let attributeHTML = '';

            if (item.attribute_value_names) {
                attribute_value_names = item.attribute_value_names;
                if (Array.isArray(attribute_value_names) && attribute_value_names.length > 0) {
                    attributeHTML +=
                        '<div class="mb-1 " style="overflow-x:auto; white-space:nowrap;">';
                    attributeHTML += attribute_value_names.join(' <span class="mx-1">|</span> ');
                    attributeHTML += '</div>';
                }
            }
            ...
            if (item_description == null || item_description == '') {
                ...
                item_description = stripHTML(item.short_description)
                    .replace(/\s+/g, ' ')
                    .replace(/\n/g, ' ')
                    .replace(/\r/g, ' ')
                    .trim();
            ...
            html += '<div class="row">' +
                ...
                '</button>' +
                '</div>' +
                '<div class="product-line-price align-self-center px-1">' + currency + ' ' + (item.qty * price).toLocaleString(undefined, { minimumFractionDigits: 2 }) + '</div>' +
