        ...
        success: function (result) {
            ...
            onMessage(messaging, (payload) => {
                ...
                if (notification.type == 'message') {
                    ...
                    var picture = '<figure class="avatar avatar-md"><img src="' + base_url + 'assets/front_end/classic/images/user.png" class="rounded"></figure>';
                    ...
                    if (notification.message_type == 'person') {
                        ...
                        if (from_id_fmc == from_id_input && chat_type == type) {
                            ...
                            // update last message preview for sender in list
                            var $itemLeft = $(".chat-person[data-id='" + from_id_fmc + "'] .text-secondary");
                            if ($itemLeft.length) { $itemLeft.text(message); }
    ...
    if (message.length > 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                // update last message preview on the left list for the active user
                var oppo = $("#opposite_user_id").val();
                var $item = $(".chat-person[data-id='" + oppo + "'] .text-secondary");
                if ($item.length) { $item.text($("#chat-input-textarea").val()); }
...
$(document).on('click', '.delete-msg-alert', function (e) {
    ...
    Notiflix.Confirm.show(
        'Are you sure?',
        'Message once deleted, you will not be able to recover that message!',
        'Yes, Delete',
        'Cancel',
        function okCb() {
            deleteMsg(msg_id);
...
$('.search_user').each(function () {
    $(this).select2({
        ajax: {
            url: base_url + 'admin/chat/search_user',
            type: "GET",
            dataType: 'json',
            delay: 250,
...
function printChat(chats, id_of_user) {
    ...
    if (0 < chats.length) {
        ...
        for (var i = 0; i < chats.length; i++) {
            ...
            picture = '<figure class="avatar avatar-md"><img src="' + base_url + 'assets/front_end/classic/images/user.png" class="rounded"></figure>';
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (type == 'person') {
                ...
                var html = '<figure class="avatar avatar-md"><img src="' + base_url + 'assets/front_end/classic/images/user.png" class="rounded"></figure>';
...
myDropzone.on('successmultiple', function (file, response) {
    ...
    // update last message preview on the left list for the active user
    var oppo2 = $("#opposite_user_id").val();
    var $item2 = $(".chat-person[data-id='" + oppo2 + "'] .text-secondary");
    if ($item2.length) { $item2.text($("#chat-input-textarea").val()); }
